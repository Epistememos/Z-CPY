...
Writes are also appended to `wal.bin` before acknowledgement — same raw bytes, sequential append, cheapest possible disk write — and fsynced before returning. On startup, torn-tail detection checks `file_size % 16 != 0` and truncates any partial packet left by a crash. If the memtable is missing or has fewer packets than the WAL, the missing packets are replayed back into the memtable before normal operation resumes.
...
- [x] Read-path benchmark: `MemTable::query` — 81.1 ns mean, 125 ns p99 over 5M packets
